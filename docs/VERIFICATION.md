...
Fixtures are stored in `test_assets/fixtures/`. They are invalidated
automatically when `FIXTURE_SCHEMA_VERSION` bumps (pipeline change). If
that happens, run `build_fixtures.py --force` to rebuild.

### Four phases

| Phase | What it tests | Gate |
|-------|--------------|------|
| 1 — Architecture correctness | Re-authored decoder vs HF Qwen2 in fp32. Random token sequence with real embedding-table inputs. | >80 dB PSNR |
...
**Key metric notes:**
- **Top-5 overlap** is the primary gate. It asks "do the same reasonable
  next tokens appear in both distributions?" A top-1 flip between
  semantically equivalent candidates (`The` vs `This`) is expected and
  acceptable under compression — top-5 overlap catches real regressions.
- **Top-1 agreement** is context only. Low top-1 with high top-5 means
  the model is re-ranking equivalent candidates, not making wrong choices.
